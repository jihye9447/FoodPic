...
public class SingUpActivity extends AppCompatActivity implements ViewPager.OnPageChangeListener,OnUserDataListener, View.OnClickListener,SignUpTask.ViewBridge {
    ...
    /*TODO LIST
    * 1. 인터페이스를 작성 >목적은 이해당액티비티가 어떤 일을 한건지 전부 정의 메소드로서 > ex) 회원가입처리 , 데이터베이스에 회원정보 기입
    * 2. SignUpPresenter 클래스를 만들어준다.
    * 3. 1번에 만들어준 각각의 인터페이스를 프리젠터 브릿지는 프리젠터가 채택하고, 뷰브릿지는 액티비티가 채택 (각각의 계획을 서로 가지고 있는 개념)
    * 4. 각각의 계획을 서로에게 전달해줘서 두클래스간의 커넥션을 만든다
    * 5. 4번의 방법은 프리젠터에서 생성자를 통해 뷰브릿지를 받아오고 뷰브릿지를 통해서 자신이 가지고 있는 프리젠터 브릿지를 넘겨준다.
    * 6. 5번중에서 뷰브릿지를 통해서 프리젠터 브릿지를 넘기는 방법은 뷰브릿지가 베이스뷰 브릿지를 상속 받고 있기때문에 베이스 뷰브릿지의 계획>프리젠터 브릿지를 넘겨주는 용도
    *       를통해서 넘겨준다.*/
    ...
    public void complete(boolean isSuccess) {
        if (isSuccess){
            ...
            Toast.makeText(this, "회원가입에 실패 했습니다. 다시 시도해 주세요", Toast.LENGTH_SHORT).show();
        }
    }
